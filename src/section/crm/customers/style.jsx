...
export const StyledHeaderTableCell = styled(({ theme, mode, item, ...other }) => <TableCell {...other} />)(({ theme, mode, item }) => ({
  ...
  top: 0,
  zIndex: 1,
  width: item.key === KEY.CUSTOMER ? '20%' : item.key === 'isActive' ? '5%' : item.key === 'createdAt' ? '5%' : `calc(100% / ${HEADER.length})`
}))
...
export const StyledTableRow = styled(({ theme, mode, index, customer, ...other }) => <TableRow {...other} />)(({ theme, mode, index, customer }) => ({
  backgroundColor:
    index % 2
      ? mode === KEY.DARK
  ...
  width: customer?.name ? '20%' : customer?.isActive.toString() ? '5%' : customer?.createdAt.toString() ? '5%' : `calc(100% / ${HEADER.length})`
}))
