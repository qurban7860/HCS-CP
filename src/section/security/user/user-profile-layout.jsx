...
const UserProfileLayout = () => {
  ...
  const { securityUser, isLoadingUser } = useSelector(state => state.user)
              ...
              <Grid container flexDirection={FLEX_DIR.ROW} pt={2} px={2}>
                <Grid item xs={12} md={8} display={FLEX.FLEX} alignItems={KEY.CENTER} mt={2}>
                  <GStyledSpanBox gap={1}>
                    ...
                    <ViewFormField variant={isDesktop ? TYPOGRAPHY.H4 : TYPOGRAPHY.H5} isLoading={isLoadingUser} isNoBg>
                      {truncate(defaultValues?.name, 50)}
                    </ViewFormField>
                  </GStyledSpanBox>
                </Grid>
                <Grid item xs={12} md={4} display={FLEX.FLEX} justifyContent={FLEX.FLEX_END}>
                  {!isDesktop && <GridViewField heading={t('role.roles.label')} isLoading={isLoadingUser} rolesChip={defaultValues?.roles} isNoBg />}
                  {renderStatusIcons()}
                </Grid>
              </Grid>
              <Grid container pl={2}>
                <Grid item xs={12} sm={12}>
                  {isDesktop && <GridViewField heading={defaultValues?.roles > 1 ? t('role.roles.label') : t('role.label')} isLoading={isLoadingUser} rolesChip={defaultValues?.roles} isNoBg />}
                </Grid>
              </Grid>
            </Grid>
            ...
            <Card {...GCardOption(themeMode)}>
              ...
              <Grid container spacing={2} p={2} pb={5}>
                <Grid item lg={8} sm={12}>
                  <CommonFieldsContainer defaultValues={defaultValues} fieldsConfig={fieldsUserConfig} isLoading={isLoadingUser} handleDialog={handleCustomerDialog} />
                </Grid>
