...
import { HEADER_ITEMS } from './users-header-items'
...
export const StyledHeaderTableCell = styled(({ theme, mode, item, ...other }) => <TableCell {...other} />)(({ theme, mode, item }) => ({
 ...
 //  width: item.key === KEY.SERIAL_NO ? '5%' : item.key === KEY.NAME ? '30%' : item.key === 'isActive' ? '5%' : `calc(100% / ${HEADER_ITEMS.length})`
}))
...
export const StyledTableRow = styled(({ theme, mode, index, machine, ...other }) => <TableRow {...other} />)(({ theme, mode, index, user }) => ({
 backgroundColor:
  index % 2
   ? mode === KEY.DARK
