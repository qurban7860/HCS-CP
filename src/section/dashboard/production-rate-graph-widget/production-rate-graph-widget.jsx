...
import { getLogRateGraphData, resetLogsRateGraphData } from 'store/slice'
import { ERPProductionRate } from 'section/log'
...
const LOG_TYPE = 'erp'
const LOG_PERIOD = 'Monthly'

const ProductionRateGraphWidget = ({ selectedMachine, setSelectedMachine }) => {
 const { isLoading, logsRateGraphData } = useSelector(state => state.log)
 ...
 useLayoutEffect(() => {
  dispatch(resetLogsRateGraphData())
 }, [dispatch])
 ...
 useEffect(() => {
  ...
  const debounce = _.debounce(() => {
   dispatch(getLogRateGraphData(customer._id, null, LOG_TYPE, LOG_PERIOD))
  }, 300)
  debounce()
  return () => debounce.cancel()
 }, [customer?._id, LOG_TYPE, dispatch])
 ...
 const handleRateGraphMachineChange = useCallback((event, newMachine) => {
  const machineId = newMachine?._id || null
  dispatch(resetLogsRateGraphData())
  dispatch(getLogRateGraphData(customer._id, machineId, LOG_TYPE, LOG_PERIOD))
 }, [])
 ...
 return (
  <Grid container mb={2}>
   <Grid item mb={2} bgcolor='background.paper' borderRadius={RADIUS.BORDER.borderRadius} xs={12}>
    <GStyledSpanBox>
     <FormHeader label={`${LOG_PERIOD} Production Rate`} />
    </GStyledSpanBox>
    <Grid
     container
       ...
       <Grid item xs={12} sm={6}>
        <Autocomplete
         ...
         onChange={(event, newMachine) => {
          ...
          handleRateGraphMachineChange(event, newMachine)
         }}
        />
       </Grid>
       <Grid item xs={12} sm={6}>
        <Grid container justifyContent={FLEX.FLEX_END}>
         <GStyledLoadingButton mode={themeMode} fullWidth={isMobile} onClick={handleRefresh} variant='filled' size='large'>
     ...
     <Grid item xs={12}>
      <ERPProductionRate timePeriod={LOG_PERIOD} customer={customer} graphLabels={graphLabels} logsGraphData={logsRateGraphData} isDashboard graphHeight={300} />
     </Grid>
    </Grid>
   </Grid>
  </Grid>
