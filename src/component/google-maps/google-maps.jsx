...
export default function GoogleMaps({ lat, lng, edit = false, machineView = false, latlongArr = [], mapHeight = '', center = '', zoom = '', machinesSites = [] }) {
 ...
 return isLoaded ? (
  <GoogleMap
        ...
        <InfoWindow
         ...
         <Grid container spacing={0} sx={{ margin: 0 }}>
          <Grid item maxWidth={300} textAlign='center'>
           <Typography variant={TYPOGRAPHY.H6} textAlign='left'>{`${infoWindowData?.serialNo} (${infoWindowData?.customerName})`}</Typography>
   ...
   {markerPositions.map((position, index) => (
    <m.div key={index}>
     <Marker key={index} position={position} icon={{ url: ASSET.MAP_MARKER, scaledSize: new window.google.maps.Size(50, 50) }} draggable={edit} ref={ref => (markerRefs.current[index] = ref)} />
    </m.div>
   ))}
  </GoogleMap>
 ) : (
