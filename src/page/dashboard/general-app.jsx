...
export default function GeneralAppPage() {
  ...
  const { count, machinesByCountry, machinesByYear, machinesByModel, erpLogs } = useSelector(
    (state) => state.count
  )
  const { activeMachineModels } = useSelector((state) => state.machinemodel)
  const { activeCategories } = useSelector((state) => state.category)
  const enviroment = GLOBAL.ENV.toLowerCase()
            ...
            <StyledGlobalCard variants={varFade().inDown}>
              <Grid container mt={2} mb={2}>
                ...
                <Grid item xs={12} sm={8} display="flex" justifyContent="flex-end" columnGap={0.5}>
                  ...
                  <Autocomplete
                    fullWidth
                    options={activeMachineModels.filter((model) =>
                      MBCCategory ? model?.category?._id === MBCCategory?._id : model
                    )}
              ...
              <Grid container mt={2} mb={2}>
                ...
                <Grid item xs={12} sm={8} display="flex" justifyContent="flex-end" columnGap={0.5}>
                  ...
                  <Autocomplete
                    fullWidth
                    options={activeMachineModels.filter((model) =>
                      MBYCategory ? model?.category?._id === MBYCategory?._id : model
                    )}
