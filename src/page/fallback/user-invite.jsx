...
import { StyledRoot, StyledContent } from 'section/auth'
...
function UserInviteLanding() {
 ...
 const ChangePassWordSchema = Yup.object().shape({
  ...
  password: Yup.string().trim().min(6, 'Password must be at least 6 characters').max(18, 'Password must be less than 18 characters').required('Password is required'),
     ...
     <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Stack spacing={2}>
       ...
       <Box sx={{ mt: 3, display: 'flex', justifyContent: 'end' }}>
        <LoadingButton size="large" type="submit" variant="contained" loading={isSubmitSuccessful || isSubmitting} sx={{ bgcolor: '#10079F', color: 'white', '&:hover': { bgcolor: '#FFA200' } }}>
         Save
        </LoadingButton>
       </Box>
      </Stack>
